...
def _empty_summary(shift: float, size: int, perturbation: bool, perturbation_strength: float, n_runs: int, dominant_break_level: int, n_break_2: int, n_break_3: int) -> dict[str, object]:
    return {
        "shift": float(shift),
        "size": int(size),
...
def _summarize_shift(shift: float, case_rows: list[dict[str, object]], size: int, refinement_levels: int, perturbation: bool, perturbation_strength: float, n_per_family: int) -> tuple[dict[str, object], list[dict[str, object]]]:
    ...
    if not n_break_2 or not n_break_3:
        return _empty_summary(shift, size, perturbation, perturbation_strength, len(case_rows), dominant_break_level, n_break_2, n_break_3), [
...
def _plot_shift_control_map(path: Path, summary_rows: list[dict[str, object]]) -> None:
    ...
    axes[3].step(shifts, split_exists, where="mid", label="split exists", color="#7c3aed")
    axes[3].step(shifts, connected, where="mid", label="connected support", color="#b45309")
    axes[3].set_ylabel("flag")
    axes[3].set_xlabel("schedule shift")
    axes[3].set_yticks([0, 1])
    axes[3].set_title("Split and Connected-Support Status")
    axes[3].legend()
    fig.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
...
def build_shift_sweep(
    ...
    for shift in shifts:
        case_rows = [
            _run_seed_case(int(seed), size, refinement_levels, float(shift), perturbation, perturbation_strength)
            for seed in seeds
        ]
        summary, counts = _summarize_shift(float(shift), case_rows, size, refinement_levels, perturbation, perturbation_strength, n_per_family)
...
def main() -> None:
    ...
    for row in summary_rows:
        print(
            ...
            f"dominant={row['dominant_break_level']} split_exists={row['split_exists']} "
